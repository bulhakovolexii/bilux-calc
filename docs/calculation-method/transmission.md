...
де $H_{tr,adj}$ — загальний коефіцієнт теплопередачі трансмісією, Вт/К, встановлений для різниці температур всередині-ззовні, визначений згідно з [розділом 3](transmission.md#3-узагальнені-коефіцієнти-теплопередачі-трансмісією);  
...
де $R_{∑}$ — опір теплопередачі що визначають згідно з ДСТУ 9191[^3] за формулою $\eqref{eq:7}$, м²·К/Вт;  
...
{: .warning }
> Формула (16) містить обидва значення $d_w$ та $d_t$ та є дійсною для випадку $d_w≥d_t$. Якщо $d_w<d_t$, тоді $d_t$ у формулі (16) замінюють на $d_w$.
